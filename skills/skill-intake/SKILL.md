...
9. For normal intake, prefer `scripts/intake_workflow.py` to run scan, report, registry, and router steps consistently.
10. If approved, install through the system `skill-installer`; do not hand-copy third-party code unless the user explicitly asks.
11. After install, confirm the installed folder name and frontmatter name.
12. Tell the user to restart Codex after any new skill installation or router update.
...
```bash
python3 ~/.codex/skills/skill-intake/scripts/intake_workflow.py \
  --url https://github.com/owner/repo/tree/main/skills/foo \
...
Use `capability_profile` in the JSON report to compare:
...
If a candidate's capability profile is fully covered by installed skills, keep it as `defer-duplicate` unless it has a clear implementation advantage.
